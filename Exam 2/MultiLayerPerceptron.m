...
classdef MultiLayerPerceptron < matlab.mixin.SetGet % ('hgsetget' for previous versions)
    ...
    properties (Access = 'public')
        ...
        InitialMarquardtParameter  = 0.01;        % Levenberg-Marquardt adjustment parameter intial value
        MaximumMarquardtParameter  = 1e10         % Levenberg-Marquardt adjustment parameter maximum value
        MarquardtFactor            = 10;          % Levenberg-Marquardt adjustment parameter evolution factor
        BayesianRegularization     = false;       % Bayesian regularization indicator
    ...
    methods (Access = 'public')
            ...
            for n = 1:length(Properties)
                ...
                switch lower(Property)
                    ...
                    case lower('TrainingAlgorithm')
                        ...
                        switch lower(Values{n}) %#ok<PROPLC>
                            case lower({'Gradient descent','Levenberg-Marquardt',...
                                        'GD',              'LM'})
                            otherwise, error('MyComponent:incorrectType',...
                                            ['Property ''TrainingAlgorithm'' shall be chosen among:\n',...
                                             ' - ''Gradient descent''        (or ''GD''),\n',...
                                             ' - ''Levenberg-Marquardt''     (or ''LM'')']);
                        end
        ...
        function Object = train(Object,Inputs,Outputs,Options)
            ...
            switch lower(Object.TrainingAlgorithm)   
                ...
                case lower({'Levenberg-Marquardt','LM'}), Object.trainLM(Inputs,Outputs);
            end
    ...
    methods (Access = 'private')
        ...
        % Method 'trainLM', Levenberg-Marquardt algorithm with or without bayesian regularization
        function Object = trainLM(Object,Inputs,Outputs)
                        
            % Split between training and validation data
            [Nt, It, Ot, Nv, Iv, Ov] = Object.splitSets(Inputs,Outputs);
            
            % Length of weigths/biases vector
            Length = ...
                Object.LengthsOfLayers(1:end-1)*Object.LengthsOfLayers(2:end)'+...
                sum(Object.LengthsOfLayers(2:end));
            
            % Preallocations
            Object.WeightsVector = zeros(Length,1);                                 % Weights/biases vector
            Object.JacobianMatrix = zeros(Object.LengthsOfLayers(end)*Nt,Length);   % Jacobian matrix
            Id = eye(Length);                                                       % Identity matrix
            
            % Initialization
            e   = 0;                                                                % Initial epoch
            mu  = Object.InitialMarquardtParameter;                                 % Initial Marquadt parameter
            k   = Object.MarquardtFactor;                                           % Marquadt parameter evolution factor
            MSE = +inf;                                                             % Initial mean square error
            
            % Initial bayesian hyper-parameters
            if Object.BayesianRegularization
                Beta  = 1;
                Alpha = 0;
            end
            
            % Initialization of control window    
            Object.TrainingStep = 'start';        
            feval(Object.UpdateFcn,Object);
            ...
            while lt(e,  Object.NumberOfEpochs) && ...
                  gt(MSE,Object.MinimumMSE) && ...
                  le(mu, Object.MaximumMarquardtParameter)
                
                % Calculation of hessian matrix and gradient
                if Status
                    
                    % Propagation of inputs vectors
                    Object.propagate(It);
                    
                    % Set of output vectors (matrix)
                    F = Object.Outputs;
                    ...
                    % Sum of errors
                    switch Object.BayesianRegularization
                        ...
                        case true
                            ...
                            % Sum of square weights/biases
                            SSW = Object.WeightsVector'*Object.WeightsVector/2;
                            
                            % Sum of sums
                            S = Beta*SSE+Alpha*SSW;
                            
                    end
                    
                    % Computation of jacobian matrix
                    Object.calculateJacobianMatrix(It,Nt,F);
                    
                    % Transposed jacobian matrix
                    Jt = Object.JacobianMatrix';
                    
                    % Quasi-hessian matrix
                    H = Jt*Object.JacobianMatrix;
                    
                end
                
                % Adjustement of weights/biases vector
                switch Object.BayesianRegularization
                    
                    case false
                        
                        % Adjustement without bayesian regularization
                        W2 = Object.WeightsVector -(H+mu*Id)\(Jt*E);
                        
                    case true
                        
                        % Adjustement with bayesian regularization
                        W2 = Object.WeightsVector - (Beta*H+(Alpha+mu)*Id) \ (Beta*Jt*E+Alpha*Object.WeightsVector);
               
                end
                
                % Multilayer perceptron update
                Object.updateWeights(W2);
                
                % Propagation with the updated weights/biases
                Object.propagate(It);
                
                % Error vector
                E2 = Object.Outputs-Ot;
                
                % Sum of errors with updated weights/biases vector
                switch Object.BayesianRegularization
                    ...
                    case true
                        ...
                        % Sum of sums
                        S2 = Beta*SSE2+Alpha*SSW2;
                        
                        % Update of bayesian hyper-parameters
                        warning('off','all');
                        Gamma = Length - Alpha*trace(inv(Beta*Jt*Object.JacobianMatrix+Alpha*Id));
                        warning('on','all');
                        Alpha = Gamma/(2*SSW2);
                        Beta  = (Nt-Gamma)/(2*SSE2);
                ...
                switch lt(S2,S)
                    ...
                    case true
                        ...
                        % Mean square error
                        switch Object.BayesianRegularization
                            case false, MSE = S2/Nt;
                            case true,  MSE = SSE2/Nt;
                        end
                        Object.MeanSquareErrors(e) = MSE;
                        
                        % Mean square error at current epoch for validation
                        Object.propagate(Iv);                
                        E = Object.Outputs-Ov;
                        Object.ValidationMeanSquareErrors(e) = sum(sum(E.^2))/2/Nv; 
                        
                        % Update of Marquardt adjustment parameter
                        mu = mu/k;
                        
                        % Update of control window 
                        Object.TrainingStep = 'update';      
                        if ~feval(Object.UpdateFcn,Object)
                            break
                        end
                    ...
                    case false
                        ...
                        % Cancellation of the last weights/biases update
                        Object.updateWeights(Object.WeightsVector);
                        
                        % Update of control window
                        Object.TrainingStep = 'cancellation';      
                        if ~feval(Object.UpdateFcn,Object)
                            break
                        end
            ...
            % Finalization of control window    
            Object.TrainingStep = 'stop';
            feval(Object.UpdateFcn,Object);
        ...
        function Continue = updateFigure(Object)
            ...
            function Elements = CreationFigure()
                ...
                switch lower(Object.TrainingAlgorithm)
                    ...
                    case lower({'LM','Levenberg-Marquardt'})
                        Title = 'Training algorithm: Levenberg-Marquardt';               
                end
